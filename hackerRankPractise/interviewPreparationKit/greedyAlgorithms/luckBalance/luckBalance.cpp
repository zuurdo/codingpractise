...
int luckBalance(int k, vector<vector<int> > contests)
{
  ...
  int notImportantContest = 0;
  for( int i = 0; i < contests.size(); i++)
  {
    if(contests[i][1] == 1)
    {
      importantContest.push_back(contests[i][0]);
    }
    else
    {
      notImportantContest += contests[i][0];
    }
  }

  std::sort(importantContest.rbegin(),importantContest.rend());

  for(int i = 0; i < importantContest.size();i++)
  {
    if( k != 0)
    {
      notImportantContest += importantContest[i];
      k--;
    }
    else
    {
